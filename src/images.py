...
def copy_image(name: str, vm: dict) -> dict:
    ...
    if images_path is None:
        logging.error(
            f"Could not find the default images path --> {images_path}")
        return {'dest_image': None, 'source_image_exists': False, 'dest_image_exists': False}

    dest_image = Path(images_path) / \
        f"{name}.{vm['storage']['disk']['format']}"

    source_image = Path(config.ARTIFACTS_PATH) / \
    ...
    if not dest_image.exists():
        if source_image.exists():
            try:
                ...
                logging.warning(
                    f"File already exists at {dest_image}. Skipping copy.")
        else:
