...
def print_message(keyword, message):
    ...
    color = keyword_colors.get(
        keyword, ""
    )  # Get the color for the keyword from the dictionary
    colored_keyword = (
        f"{color}{keyword}:{colorama.Style.RESET_ALL}" if color else keyword
    )  # Add color to the keyword if it exists in the dictionary
